...
- **Only relay-proxy/`REMOTE`-mode evidence is produced -- this is a known gap, not a
  design choice.** The design spec calls for embedded/library-mode (`IN_PROCESS`) evidence
  as Gate 1's *primary* requirement, with relay-proxy mode only as a secondary,
  separately-labelled run (relay-proxy mode is structurally a sidecar-shaped network hop --
  see `docs/feature-flags-research-gaps-v2.md` Thread 5 -- with a different cost profile).
  ...
  class javadoc for the detail). Whoever advances Slice B needs to either bump this
  module's GOFF provider dependency to `>= 1.0.0` and re-verify this harness against the
  newer API, or get the design spec's Gate 1 requirement explicitly revised to accept
  relay-proxy-only evidence. Only read the numbers this harness produces as relay-proxy
  numbers -- never as a stand-in for embedded-mode evidence.
